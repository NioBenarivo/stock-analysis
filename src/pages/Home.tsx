...
function CompanyCard({ stock }: { stock: StockAnalysis }) {
  return (
    <Link
      to={`/company/${stock.id}`}
      className="flex flex-col gap-4 p-5 bg-white rounded-2xl border border-slate-200 hover:border-blue-300 hover:shadow-md transition-all group"
    >
      <div className="flex items-center gap-3">
        <CompanyLogo domain={stock.logoDomain} ticker={stock.ticker} />
        <span className="text-sm font-semibold text-slate-500">{stock.ticker}</span>
      </div>

      <div>
        <p className="text-slate-900 font-semibold text-base leading-snug group-hover:text-blue-600 transition-colors">
          {stock.title}
        </p>
        <p className="text-slate-500 text-sm mt-1 leading-snug">{stock.subtitle}</p>
      </div>
    </Link>
  )
}
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto px-10 py-10">
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-slate-900">All Companies</h1>
          <p className="text-slate-500 text-sm mt-1">{publicAnalyses.length} analyses</p>
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-20 text-slate-400 text-sm">No companies match your search.</div>
        ) : (
