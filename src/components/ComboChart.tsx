    ...
    <div className="bg-white border border-slate-200 rounded-lg px-3 py-2 shadow-lg text-xs space-y-1">
      {payload.map((p) => (
        <div key={p.dataKey} className="flex items-center gap-2">
          ...
          <span className="font-semibold text-slate-900 ml-auto pl-4">
            {p.value}{leftIds.includes(p.dataKey) ? leftUnit : rightUnit}
          </span>
        </div>
      ))}
    </div>
              ...
              {years.map((year) => (
                <tr key={year} className="border-b border-slate-50 last:border-0">
                  ...
                  {allMetrics.map((m) => {
                    ...
                    return (
                      <td key={m.id} className="py-1.5 text-right font-medium text-slate-900">
        ...
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
            ...
            <YAxis
              ...
              domain={['auto', 'auto']}
            />
            <Tooltip content={<CustomTooltip leftUnit={leftUnit} rightUnit={rightUnit} leftIds={leftIds} />} />
            ...
            {rightMetrics.map((m, i) => (
              <Line
                ...
                dataKey={m.id}
                name={m.label}
                stroke={COLORS[(leftMetrics.length + i) % COLORS.length]}
                strokeWidth={2}
                dot={false}
