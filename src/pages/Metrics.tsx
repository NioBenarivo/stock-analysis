...
function StockCard({ stock }: { stock: StockAnalysis }) {
  ...
  return (
    <Link
      to={`/metrics/${stock.id}`}
      className="flex flex-col gap-3 p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-blue-300 dark:hover:border-blue-700 hover:shadow-md transition-all group"
    >
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1.5">
        {latest.slice(0, 4).map((m) => (
          <div key={m.label}>
            <p className="text-xs text-slate-400 dark:text-slate-500 truncate">{m.label}</p>
