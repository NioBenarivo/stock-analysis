...
export default function ComparisonChart({ label, unit, type = 'line', series }: Props) {
  ...
  function renderChart(h: number) {
    const axes = (
      <>
        ...
        />
        <Tooltip
          formatter={(v, name) => [fmtValue(Number(v), unit), name]}
          contentStyle={{
    ...
    const margin = { top: 24, right: 28, bottom: 0, left: 0 }
